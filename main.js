...
async function main() {
  ...
  if (queryIds.length > proxies.length && settings.USE_PROXY) {
    console.log(chalk.red("The number of proxies and data must be equal.."));
    console.log(chalk.blue(`Data: ${queryIds.length}`));
    console.log(chalk.blue(`Proxy: ${proxies.length}`));
    process.exit(1);
  }
...
`);
  ...
  while (true) {
    ...
    // 24 နာရီ စောင့်အောင် ပြင်ထားတယ်
    await sleep(3);
    const completedTime = new Date().toLocaleString();
    const nextRunTime = new Date(Date.now() + 24 * 60 * 60 * 1000).toLocaleString(); // 24 နာရီ ထည့်ပေါင်းတယ်
    console.log(`
 █████╗ ██████╗ ██████╗     ███╗   ██╗ ██████╗ ██████╗ ███████╗
██╔══██╗██╔══██╗██╔══██╗    ████╗  ██║██╔═══██╗██╔══██╗██╔════╝
███████║██║  ██║██████╔╝    ██╔██╗ ██║██║   ██║██║  ██║█████╗  
...
╚═╝  ╚═╝╚═════╝ ╚═════╝     ╚═╝  ╚═══╝ ╚═════╝ ╚═════╝ ╚══════╝   
            ${chalk.yellow('magicnewton')}                
📢  ${chalk.yellow(`Completed all accounts at ${completedTime}`)}
📅  ${chalk.green(`Next run scheduled at ${nextRunTime}`)}
`);
    console.log(`============= Waiting 24 hours before next run =============`.magenta);
    await sleep(24 * 60 * 60); // 24 နာရီ (86400 စက္ကန့်) စောင့်တယ်
  }
}
